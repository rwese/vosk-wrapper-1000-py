...
from xdg_paths import get_hooks_dir, get_default_model_path
...
def audio_callback(indata, frames, time, status):
    ...
    if listening:
        try:
            # Convert buffer to numpy array (sounddevice gives us float32 or int16)
            if indata.dtype == np.float32:
                audio_data = (indata * 32767).astype(np.int16)
            else:
                audio_data = indata.copy()  # Already int16
            ...
            if noise_filter_enabled:
                ...
                audio_data = np.clip(audio_float * 32767, -32768, 32767).astype(
                    np.int16
                )

            # Resample if needed using soxr for high-quality streaming
            if device_rate != model_rate:
                # Convert to float for soxr
                audio_float = audio_data.astype(np.float32) / 32768.0

                # Use soxr streaming resampler
                if soxr_resampler is None:
                    print(
                        f"Creating soxr resampler: {device_rate} -> {model_rate} Hz",
                        file=sys.stderr,
                    )
                    soxr_resampler = soxr.ResampleStream(
                        rate_in=device_rate,
                        rate_out=model_rate,
                        num_channels=1,
                ...
                # Resample using soxr
                resampled_float = soxr_resampler.resample_chunk(
                    audio_float.reshape(-1, 1), last=False
                )

                # Convert back to int16
                final_audio = np.clip(resampled_float * 32767, -32768, 32767).astype(
                    np.int16
                )
            else:
            ...
            if recording_file:
                recording_file.write(final_audio.tobytes())

            # Queue for Vosk processing
            audio_queue.put_nowait(bytes(final_audio))
        except queue.Full:
    ...
    if listening:
        try:
            ...
            # Resample if needed using efficient methods
            if device_rate != model_rate:
                # Convert to float for resampling
                audio_float = audio_data.astype(np.float32) / 32768.0

                # Use efficient resampling based on ratio
                if device_rate % model_rate == 0:
                    # Simple integer ratio - use decimate for efficiency
                    ratio = device_rate // model_rate
                    resampled = scipy_signal.decimate(audio_float, ratio, ftype="fir")
                else:
                    # Complex ratio - use resample_poly for better quality
                    resampled = scipy_signal.resample_poly(
                        audio_float, model_rate, device_rate
                    )

                # Convert back to int16
                audio_int16 = np.clip(resampled * 32767, -32768, 32767).astype(np.int16)
                final_audio = audio_int16
            else:
                final_audio = audio_data
    ...
    if listening:
        try:
            ...
            if noise_filter_enabled:
                ...
                audio_float = nr.reduce_noise(
                    y=audio_float, sr=device_rate, stationary=True
                )
                # Convert back to int16
                audio_data = np.clip(audio_float * 32767, -32768, 32767).astype(
                    np.int16
...
def get_device_info(device_arg):
    ...
    if device_arg is None:
        ...
        try:
            ...
            print(
                f"Searching for device with name containing: '{device_arg}'...",
                file=sys.stderr,
            )
            ...
            for i, device in enumerate(devices):
                ...
                if (
                    ...
                    and device_arg.lower() in device["name"].lower()
                ):
            ...
            if device_id is None:
                print(
                    f"Error: No input device found matching '{device_arg}'",
                    file=sys.stderr,
                )
                sys.exit(1)
...
def run_service(args):
    ...
    # Setup recording file if specified (model_rate will be set later)
    recording_file = None
    if args.record_audio:
        # We'll set up the recording file after model_rate is known
        pass
    ...
    if not os.path.exists(args.model):
        print(
            f"Model not found at '{args.model}'. Please run vosk-download-model-1000 or specify correct path.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.samplerate == 16000 and detected_model_rate != 16000:
        ...
        print(
            f"Auto-detected model sample rate: {model_sample_rate} Hz", file=sys.stderr
        )
    else:
        model_sample_rate = args.samplerate
        if args.samplerate != detected_model_rate:
            print(
                f"Warning: Model expects {detected_model_rate} Hz but using {args.samplerate} Hz",
                file=sys.stderr,
            )
    ...
    is_compatible, compatibility_msg = validate_device_compatibility(
        device_id, model_sample_rate
    )
    ...
    # Setup recording file now that we know model_rate
    if args.record_audio:
        try:
            import wave

            recording_file = wave.open(args.record_audio, "wb")
            recording_file.setnchannels(1)  # Mono
            recording_file.setsampwidth(2)  # 16-bit
            recording_file.setframerate(model_rate)  # Model's sample rate
            print(f"Recording processed audio to: {args.record_audio}", file=sys.stderr)
        except Exception as e:
            print(
                f"Error opening recording file {args.record_audio}: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if device_rate != model_rate:
        print(
            f"Device sample rate: {device_rate} Hz, Model rate: {model_rate} Hz",
            file=sys.stderr,
        )
        print(
            f"Audio will be resampled from {device_rate} Hz to {model_rate} Hz",
            file=sys.stderr,
        )
    else:
        print(
            f"Audio sample rate: {model_rate} Hz (no resampling needed)",
            file=sys.stderr,
        )
    ...
    if noise_filter_enabled:
        print(
            f"Noise filtering: ENABLED (strength={noise_reduction_strength}, stationary={stationary_noise})",
            file=sys.stderr,
        )
    else:
    ...
    try:
        while running:
            ...
            if listening and stream is None:
                ...
                print(
                    f"DEBUG: device_rate={device_rate}, device_id={device_id}",
                    file=sys.stderr,
                )
                sys.stderr.flush()
                ...
                try:
                    ...
                    print(
                        "DEBUG: RawInputStream object created, now starting...",
                        file=sys.stderr,
                    )
                    sys.stderr.flush()
                    ...
                    print(
                        "DEBUG: RawInputStream started successfully!", file=sys.stderr
                    )
                    sys.stderr.flush()
                    print(
                        f"Microphone stream started at {device_rate} Hz (using soxr resampling to {model_rate} Hz).",
                        file=sys.stderr,
                    )
                    sys.stderr.flush()
        ...
        if recording_file:
            ...
            print(f"Audio recording saved to: {args.record_audio}", file=sys.stderr)

        # Clean up soxr resampler
        global soxr_resampler
        if soxr_resampler:
            soxr_resampler = None
            print("Cleaned up soxr resampler", file=sys.stderr)
...
For more information, visit: https://github.com/rwese/vosk-wrapper-1000-py
    ...
    daemon_parser = subparsers.add_parser(
        "daemon", help="Run the voice recognition service as daemon"
    )
    ...
    start_parser.add_argument(
        "name", nargs="?", default="default", help="Instance name (default: default)"
    )
