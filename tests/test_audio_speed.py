...
def analyze_audio_file(filepath):
    ...
    with wave.open(str(filepath), "rb") as wf:
        ...
        print("If audio sounds wrong, the actual sample rate might be different:")
        print("Possible actual sample rates and their durations:")
        ...
        for actual_rate in common_rates:
            ...
            if abs(speed_factor - 1.0) < 0.01:
                print("(normal speed - matches header)")
            elif speed_factor > 1.0:
                print(
                    f"(sounds {speed_factor:.2f}x TOO FAST if header is {header_rate} Hz)"
                )
            else:
                print(
        ...
        print("  - Audio sounds TOO SLOW → actual rate is LOWER than header rate")
        print("  - Audio sounds TOO FAST → actual rate is HIGHER than header rate")
        print()
